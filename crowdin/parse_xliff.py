...
class ValidationResult:
    ...
    def add_issue(self, locale: str, string_key: str, issue_type: str, message: str, severity: str = "error"):
        self.issues.append(ValidationIssue(locale, string_key, issue_type, message, severity))
...
def get_string_value(trans_data: Dict[str, Any]) -> str:
    """Get the string value from translation data, handling both regular strings and plurals."""
...
def find_invalid_angle_brackets(text: str) -> List[str]:
    ...
    for i, char in enumerate(text):
        if char == '<':
            ...
            valid_tag = re.match(r'</?(?:b|br|span)(?:\s*/)?>', remaining, re.IGNORECASE)
            if not valid_tag:
...
def validate_all_translations(parsed_locales: Dict[str, Any], source_locale: str) -> ValidationResult:
    ...
    # Get source (English) translations
    if source_locale not in parsed_locales:
        result.add_issue(source_locale, "", "missing_source", f"Source locale '{source_locale}' not found")
        return result
...
def print_validation_summary(result: ValidationResult):
    ...
    # Print details by locale (limited)
    ...
    for locale, issues in sorted(by_locale.items()):
        ...
        if errors or warnings:
            ...
            for issue in issues[:3]:
                ...
                print(f"    {color}- {issue.string_key}: {issue.message}{Style.RESET_ALL}")
            if len(issues) > 3:
...
def parse_all_xliff_files(input_directory: str, skip_validation: bool = False) -> Tuple[Dict[str, Any], ValidationResult]:
    ...
    if not skip_validation:
        ...
        validation_result = validate_all_translations(parsed_locales, source_locale)
...
def main():
    ...
    try:
        ...
        if args.validation_report and validation_result.issues:
            ...
            os.makedirs(os.path.dirname(args.validation_report) or '.', exist_ok=True)
        ...
        # Exit with error if validation failed and flag is set
