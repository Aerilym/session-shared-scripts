...
# Semaphore to limit concurrent API requests (Crowdin limit: 20)
api_semaphore = Semaphore(20)


def make_request_with_retry(method: str, url: str, **kwargs) -> requests.Response:
    """
    Make an HTTP request with exponential backoff retry on rate limiting (429) errors.
    """
    last_exception = None
    
    for attempt in range(MAX_RETRIES):
        try:
            with api_semaphore:  # Ensure we don't exceed 20 concurrent requests
                if method.upper() == 'GET':
                    response = requests.get(url, timeout=REQUEST_TIMEOUT_S, **kwargs)
                elif method.upper() == 'POST':
                    response = requests.post(url, timeout=REQUEST_TIMEOUT_S, **kwargs)
                else:
                ...
                if response.status_code == 429:
                    ...
                    if VERBOSE:
                        print(f"\n{Fore.YELLOW}⚠️  Rate limited, waiting {retry_after}s before retry...{Style.RESET_ALL}")
            ...
            if VERBOSE:
                print(f"\n{Fore.YELLOW}⚠️  Request failed, retrying in {delay}s... ({e}){Style.RESET_ALL}")
            time.sleep(delay)
    
    raise last_exception or Exception(f"Request failed after {MAX_RETRIES} retries")
...
def check_error(response, context=""):
    ...
    if response.status_code != 200:
        ...
        raise Exception(f"{context}: {error_msg} (Code: {response.status_code})")
...
def export_and_download_language(language: dict, is_source: bool = False) -> str:
    """
    Export and download translations for a single language.
    Returns the locale string on success.
    """
    ...
    # Request export with retry logic
    export_payload = {
    ...
    with progress_lock:
        completed_count += 1
        print(f"\033[2K{Fore.WHITE}⏳ Downloaded {completed_count}/{total_count} translations...{Style.RESET_ALL}", end='\r')
...
def main():
    """
    Main Function
    Fetch crowdin project info, and download all translations in parallel.
    """
    ...
    # Prepare all languages (source + targets)
